...
const createInnerSlider = (props, forceUpdate) => {
  const innerSlider = {
    props,
    list: null,
    track: null,
    state: {
      ...
      targetSlide: props.initialSlide ? props.initialSlide : 0,
    ...
    pendingState: null,
    pendingSetStateCallbacks: [],
    updateId: 0,
    renderedUpdateId: 0,

    // Preserve class-style setState timing while rendering through a function component.
    setState(state, callback) {
      const prevState = this.pendingState || this.state;
      const nextState =
        typeof state === "function" ? state(prevState, this.props) : state;

      if (nextState) {
        this.pendingState = { ...prevState, ...nextState };
      }

      this.updateId += 1;
      if (callback) {
        this.pendingSetStateCallbacks.push({
          callback,
    ...
    flushSetStateCallbacks() {
      const callbacks = [];
      this.pendingSetStateCallbacks = this.pendingSetStateCallbacks.filter(
        (item) => {
          if (item.updateId <= this.renderedUpdateId) {
    ...
    onWindowResized(setTrackStyle) {
      ...
      this.debouncedResize = debounce(50, () =>
  ...
  [
    ...
    innerSlider[method] = innerSlider[method].bind(innerSlider);
  });

  const ssrState = innerSlider.ssrInit();
  innerSlider.state = { ...innerSlider.state, ...ssrState };

  return innerSlider;
};

export const InnerSlider = React.forwardRef((props, ref) => {
  ...
  if (!innerSliderRef.current) {
    innerSliderRef.current = createInnerSlider(props, forceUpdate);
  }

  const innerSlider = innerSliderRef.current;
  innerSlider.props = props;
  if (innerSlider.pendingState) {
    innerSlider.state = innerSlider.pendingState;
    innerSlider.pendingState = null;
  }
  innerSlider.renderedUpdateId = innerSlider.updateId;

  React.useImperativeHandle(ref, () => innerSlider, []);

  useIsomorphicLayoutEffect(() => {
    innerSlider.componentDidMount();
    return () => {
      innerSlider.componentWillUnmount();
    };
  }, []);

  useIsomorphicLayoutEffect(() => {
    if (didUpdateRef.current) {
      innerSlider.componentDidUpdate(prevPropsRef.current);
    } else {
  ...
  useIsomorphicLayoutEffect(() => {
    innerSlider.flushSetStateCallbacks();
  });

  return innerSlider.render();
});
